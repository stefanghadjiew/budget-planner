...
async function encode(payload: JwtPayload): Promise<string | undefined> {
  const certificate = await readPrivateKey();
  if (!certificate) throw new InternalError();
  // @ts-ignore
  return promisify(sign)({ ...payload }, certificate, { algorithm: 'RS256' });
}
